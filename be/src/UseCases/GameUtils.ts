...
export default class Game {
    ...
    public countTouch(cards: Card[], sampleCard: Card): number {
        ...
        let suitsQtys: { Basto?: number; Espada?: number; Copa?: number; Oro?: number } = cards
            .map(card => card.suit)
            .reduce((prev, curr) => {
                if (prev[Suit[curr]]) {
                    prev[Suit[curr]]++;
        ...
        if (Math.max(...Object.values(suitsQtys)) === 2) {
            const suitName = Object.keys(suitsQtys).find(suitName => suitsQtys[suitName] === 2);
            const cardsOfThatSuit = cards.filter(card => card.suit === Suit[suitName]);
            return (
                20 +
